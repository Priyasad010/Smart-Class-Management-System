...
/**
 * 🗑️ Deletes all Spam inquiries.
 */
exports.bulkDeleteSpam = async (req, res) => {
  try {
    const result = await db.pool.query("DELETE FROM Contact_Messages WHERE status = 'Spam' RETURNING message_id");
    await auditService.logAction(req.user.userId, req.user.role, 'DELETE', 'Contact_Message', null, `Bulk deleted ${result.rows.length} spam messages.`);
    res.status(200).json({ message: `Spam පණිවිඩ ${result.rows.length} මකා දමන ලදී!` });
  } catch (error) {
