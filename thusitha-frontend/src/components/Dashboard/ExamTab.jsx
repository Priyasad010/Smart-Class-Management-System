...
const ExamTab = ({ courses, onCreateExam, onUpdateExam, onDeleteExam }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setFormData({ ...formData, exam_name: '', exam_date: '', total_marks: 100, pass_percentage: 50 });
    fetchExams(); // Refresh list after creation
  };
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      ...
      if (response.ok) showNotification('ලකුණු සාර්ථකව ඇතුළත් කළා!');
    ...
    <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: '350px', backgroundColor: 'white', padding: '25px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
        ...
        <form onSubmit={handleExcelUpload}>
          ...
          <select id="exam_for_marks" style={inputStyle} value={selectedExamForMarks} onChange={(e) => setSelectedExamForMarks(e.target.value)} required>
          ...
          <input id="marks_file" type="file" accept=".xlsx, .xls" style={inputStyle} onChange={(e) => setExcelFile(e.target.files[0])} required />
          
          <button type="submit" disabled={uploading} style={{ width: '100%', padding: '12px', backgroundColor: '#2e7d32', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>
            {uploading ? 'පූරණය වෙමින්...' : '📤 ලකුණු ඇතුළත් කරන්න'}
          </button>
        </form>
      </div>
