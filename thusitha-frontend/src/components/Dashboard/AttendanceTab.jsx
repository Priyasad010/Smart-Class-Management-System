...
const AttendanceTab = ({ students, courses, selectedCourse, onCourseChange, selectedDate, onDateChange, attendanceRecords, onCheckboxChange, onSave, loading }) => {
  ...
  return (
    <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f9f9f9', borderRadius: '8px', display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        <div style={{ flex: '1', minWidth: '250px' }}>
          <label htmlFor="attendanceCourse" style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>පාඨමාලාව තෝරන්න (Select Class)</label>
          <select 
            id="attendanceCourse"
        ...
        <div style={{ flex: '1', minWidth: '200px' }}>
          <label htmlFor="attendanceDate" style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>දිනය (Date)</label>
          <input 
            type="date"
            id="attendanceDate"
            value={selectedDate}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f5f5f5', textAlign: 'left' }}>
              ...
              <th style={{ padding: '12px' }}>සිසුවාගේ නම</th>
              <th style={{ padding: '12px', textAlign: 'center' }}>තත්ත්වය (Status)</th>
            </tr>
          </thead>
          <tbody>
            {students.length > 0 ? students.map((student, index) => {
              const studentKey = student._id || student.studentId;
              const isPresent = !!attendanceRecords[studentKey];
              return (
                <tr key={`log-${studentKey || index}`} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '12px', fontWeight: 'bold' }}>{student.studentId || 'N/A'}</td>
                  <td style={{ padding: '12px' }}>{student.name}</td>
                  <td style={{ padding: '12px', textAlign: 'center', color: isPresent ? 'green' : 'red', fontWeight: 'bold' }}>
                    {isPresent ? 'Present (පැමිණ ඇත)' : 'Absent (නොපැමිණියේය)'}
                  </td>
                </tr>
              );
            }) : (
              <tr><td colSpan="3" style={{ padding: '20px', textAlign: 'center', color: '#666' }}>මෙම පාඨමාලාවට සිසුන් ලියාපදිංචි වී නොමැත. (No students enrolled)</td></tr>
            )}
          </tbody>
        </table>
      ) : (
