...
const ContactTab = ({ messages, onMarkRead, onMarkSpam, onRecoverFromSpam, onBulkDeleteSpam, onMarkAllRead, templates, onToggleImportant, onQuickReply }) => {
  const [filter, setFilter] = useState('Inbox'); // 'Inbox', 'Spam', 'All'
    ...
    <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
        ...
        {filteredMessages.map((msg) => (
          <div key={msg.message_id} style={{ 
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              {msg.status !== 'Spam' && (
                <button 
                  onClick={() => { setSelectedMsg(msg); setReplyText(''); setShowReplyModal(true); }}
              ...
              <button 
                onClick={() => msg.status === 'Spam' ? onRecoverFromSpam(msg.message_id) : onMarkSpam(msg.message_id)}
              ...
              >
                {msg.status === 'Spam' ? 'Recover from Spam' : 'Mark as Spam'}
              </button>
            </div>
          </div>
        ))}
      ...
      {showReplyModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1300 }}>
          <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', width: '480px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            ...
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '13px', color: '#333' }}>පණිවිඩය සංස්කරණය කරන්න (Edit Message):</label>
            <textarea
              style={{ width: '100%', height: '120px', padding: '10px', borderRadius: '6px', border: '1px solid #ccc', marginBottom: '15px', resize: 'vertical', boxSizing: 'border-box', fontSize: '14px' }}
              value={replyText}
              onChange={(e) => setReplyText(e.target.value)}
              placeholder="මෙතැන ඔබේ පණිවිඩය ලියන්න හෝ පහතින් සැකිල්ලක් (Template) තෝරන්න..."
            />

            <div style={{ marginBottom: '15px' }}>
              <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#666', display: 'block', marginBottom: '8px' }}>💡 සැකිල්ලක් තෝරන්න (Select Template to use/edit):</span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '150px', overflowY: 'auto', border: '1px solid #eee', padding: '8px', borderRadius: '6px', backgroundColor: '#fcfcfc' }}>
                {templates.length === 0 && <span style={{ fontSize: '12px', color: '#999' }}>සැකිලි කිසිවක් නැත. පද්ධති සැකසුම් වලින් එකතු කරන්න.</span>}
                {templates.map((template) => (
                  <button 
                    key={template}
                    type="button"
                    onClick={() => setReplyText(template)}
                    style={{ textAlign: 'left', padding: '8px 12px', border: '1px solid #ddd', borderRadius: '4px', cursor: 'pointer', backgroundColor: '#fff', fontSize: '12px', color: '#333' }}
                  >
                    {template}
                  </button>
                ))}
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              <button 
                ...
                style={{ flex: 1, padding: '10px', background: 'none', border: '1px solid #ccc', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
              >
                Cancel
              </button>
              <button 
                onClick={() => { 
                  ...
                  onQuickReply(selectedMsg, replyText); 
                ...
                style={{ 
                  flex: 1, 
                  padding: '10px', 
                  backgroundColor: replyText.trim() ? '#2e7d32' : '#ccc', 
                  color: 'white', 
                  border: 'none', 
                  borderRadius: '6px', 
