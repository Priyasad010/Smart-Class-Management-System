...
const ClassTab = ({ 
  ...
  onCreateCourse, onDeleteCourse,
  onCreateSubject, onDeleteSubject,
  onCreateHall, onDeleteHall
}) => {
  ...
  const [newCourse, setNewCourse] = useState({ course_name: '', description: '', fee: '' });
  ...
  return (
    <div style={{ backgroundColor: '#f9f9f9', padding: '20px', borderRadius: '10px' }}>
      <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', borderBottom: '1px solid #ddd' }}>
        ...
        <button style={tabStyle(activeSubTab === 'courses')} onClick={() => setActiveSubTab('courses')}>🎓 පාඨමාලා</button>
      ...
      {activeSubTab === 'courses' && (
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '300px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            <h3 style={{ color: '#1a237e' }}>➕ නව පාඨමාලාවක්</h3>
            <form onSubmit={(e) => { e.preventDefault(); onCreateCourse(newCourse); setNewCourse({ course_name: '', monthly_fee: '' }); }}>
              <label>පාඨමාලාවේ නම</label>
              ...
              <button style={{ width: '100%', padding: '10px', backgroundColor: '#1a237e', color: 'white', border: 'none', borderRadius: '5px' }}>සුරකින්න</button>
            </form>
          </div>
          <div style={{ flex: 2, minWidth: '400px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            <h3 style={{ color: '#1a237e' }}>පාඨමාලා ලැයිස්තුව</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: '#f5f5f5', textAlign: 'left' }}><th>නම</th><th>විස්තරය</th><th>ගාස්තුව</th><th>ක්‍රියාමාර්ග</th></tr></thead>
              <tbody>
                {courses.map(c => (
                  <tr key={c.course_id} style={{ borderBottom: '1px solid #eee' }}>
                    <td>{c.course_name}</td><td>{c.description}</td><td>{c.fee}</td>
                    <td><button onClick={() => onDeleteCourse(c.course_id)} style={{ padding: '5px', background: '#ffebee', color: '#d32f2f', border: 'none', borderRadius: '3px' }}>Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeSubTab === 'subjects' && (
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '300px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            <h3 style={{ color: '#1a237e' }}>➕ නව විෂයයක්</h3>
            <form onSubmit={(e) => { e.preventDefault(); onCreateSubject(newSubject); setNewSubject({ subject_name: '', description: '' }); }}>
          ...
          <div style={{ flex: 2, minWidth: '400px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: '#f5f5f5', textAlign: 'left' }}><th>නම</th><th>විස්තරය</th><th>ක්‍රියාමාර්ග</th></tr></thead>
              <tbody>
                {subjects.map(s => (
                  <tr key={s.subject_id} style={{ borderBottom: '1px solid #eee' }}>
                    <td>{s.subject_name}</td><td>{s.description}</td>
                    <td><button onClick={() => onDeleteSubject(s.subject_id)} style={{ padding: '5px', background: '#ffebee', color: '#d32f2f', border: 'none', borderRadius: '3px' }}>Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activeSubTab === 'halls' && (
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '300px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            <h3 style={{ color: '#1a237e' }}>➕ නව ශාලාවක්</h3>
            <form onSubmit={(e) => { e.preventDefault(); onCreateHall(newHall); setNewHall({ hall_name: '', capacity: '' }); }}>
          ...
          <div style={{ flex: 2, minWidth: '400px', backgroundColor: 'white', padding: '25px', borderRadius: '10px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: '#f5f5f5', textAlign: 'left' }}><th>නම</th><th>ධාරිතාව</th><th>ක්‍රියාමාර්ග</th></tr></thead>
              <tbody>
                {halls.map(h => (
                  <tr key={h.hall_id} style={{ borderBottom: '1px solid #eee' }}>
                    <td>{h.hall_name}</td><td>{h.capacity}</td>
                    <td><button onClick={() => onDeleteHall(h.hall_id)} style={{ padding: '5px', background: '#ffebee', color: '#d32f2f', border: 'none', borderRadius: '3px' }}>Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
